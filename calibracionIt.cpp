...
bool findRingsGrid2( Mat &view, Mat &binarized, Mat &morphology, Mat &ellipses, Mat &result, RotatedRect &minRect, Size &boardSize, vector<Point2f> &pointBuf);
...
for (int ite = 0; ite < 20; ++ite)
{
    ...
    for(;;){
         ...
         if(found && s.calibrationPattern== Settings::RINGS_GRID)
        {
            ...
            temp1 = view.clone();
            if (s.useFisheye)
                cv::fisheye::undistortImage(temp1, view, cameraMatrixA, distCoeffsA);
            else
                undistort(temp1, view, cameraMatrixA, distCoeffsA);
            ...
            perspectiveTransform( pointBuf, new_vertices, H);
            //for (int i = 0; i < pointBuf.size(); ++i)
            {
                //circle(rotated, new_vertices[i], 12, Scalar(0, 0, 0), -1, 8, 0);
            }


            vector<Point2f> pointBuf_Aux, pointBof_result;
            bool found2 = findRingsGrid2( rotated, binarized, morphology, ellipses, result, minRect, s.boardSize, pointBuf_Aux);
            //imshow("aa", binarized);
            if (found2)
            {
                //drawChessboardCorners( rotated, s.boardSize, Mat(pointBuf_Aux), found2 );
            }
            ...
            if (found2)
            {
                float x; 
                float y;
                for (int i = 0; i < pointBuf_Aux.size(); ++i)
                {
                    x = (new_vertices[i].x + pointBuf_Aux[i].x)/2;
                    y = (new_vertices[i].y + pointBuf_Aux[i].y)/2;
            ...
            drawChessboardCorners( rotated, s.boardSize, Mat(pointBof_result), found );

            //return point to original image

            imshow("FRONTO-PARALLEL IMAGE", rotated);

            perspectiveTransform( pointBof_result, pointBof_result, H.inv());
            Mat imgOriginal = view.clone();
            drawChessboardCorners( imgOriginal, s.boardSize, Mat(pointBof_result), found );
            imshow("Return IMAGE", imgOriginal);

            pointBuf.clear();
            pointBuf = pointBof_result;
    ...
    //PARAMETERS UPDATE
    cameraMatrixA = cameraMatrix.clone();
    distCoeffsA = distCoeffs.clone();
...
{
    int ellipseCount = 0;
    gridDetection2(view, binarized, morphology, ellipses, result, minRect, ellipseCount, pointBuf);
